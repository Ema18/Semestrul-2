...
void UI::printWatchlistMenu()
{
	...
	cout << "\t 4 - Store in file." << endl;
	cout << "\t 5 - Open watchlist file." << endl;
	cout << "\t 0 - Back." << endl;
}
	...
	else
	{
		for (int i = 0; i < suggestions.size(); i++)
		{
			...
			else if (command == 2)
			{
				...
				int pos = this->ctrl.getRepo().getPosition(title);
...
void UI::storeInFile()
{
	int type = typeFile();
	if (type == 1)
		this->wlist->writeToFile1("Movies.csv");
	else if (type == 2)
		this->wlist->writeToFile1("Movies.html");
	else 
		return;
}
